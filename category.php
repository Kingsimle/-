...
if (_opt('disable_cat_carousels') != "checked") {
	...
	if (!$is_fixed) {
		$args = array('cat' => $cat, 'posts_per_page' => 20, 'orderby' => 'rand', 'ignore_sticky_posts' => true);
...
?><div class="col-xs-12"><div class="row"><div class="row postLists <?php
echo $show_type;
?> <?php
...
while (have_posts()) {
	the_post();
	?><div class="col-xxs-6 col-xs-4 col-sm-6 col-md-4 col-lg-3 post-card-wrapper"><?php
	include 'assets/template/postlist-post.php';
	?></div><?php
}
?></div></div></div></div><div class="<?php
echo $rightClass;
?>"><div class="row"><div class="sidebar"><div manual-template="sidebarMenu"></div><div manual-template="sidebar" height-from="postLists"></div></div></div></div></div></div><?php
echo wp_nav($p = 2, $showSummary = false, $showPrevNext = true, $style = 'menu', $container = "panda_pagi' pandaTab active-class='.active' sub-class='.sub-menu' sub-trigger='' auto-scrolling='");
get_footer();