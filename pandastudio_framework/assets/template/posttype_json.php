...
foreach ($myPostTypes['posttypes'] as $postType) {
    ...
    if ($postType['allow_categorys']) {
        add_action('init', function () use($postType) {
            register_taxonomy($postType['type'] . '-category', $postType['type'], array('labels' => array('name' => $postType['name'] . '分类目录', 'add_new_item' => '添加新分类目录', 'new_item_name' => '分类名称'), 'show_ui' => true, 'show_in_rest' => true, 'show_tagcloud' => false, 'hierarchical' => true));
        });
    }
    if ($postType['allow_tags']) {
        add_action('init', function () use($postType) {
            register_taxonomy($postType['type'] . '-tag', $postType['type'], array('labels' => array('name' => $postType['name'] . '标签', 'add_new_item' => '添加新标签', 'new_item_name' => '标签名称'), 'show_ui' => true, 'show_in_rest' => true, 'show_tagcloud' => true, 'hierarchical' => false));
        });
    }
    if ($postType['custom_taxonomies']) {
        foreach ($postType['custom_taxonomies'] as $item) {
            add_action('init', function () use($postType, $item) {
                register_taxonomy($item['taxonomy'], $postType['type'], array('labels' => array('name' => $item['label_name'], 'add_new_item' => $item['label_add_new_item'], 'new_item_name' => $item['label_new_item_name']), 'show_ui' => $item['show_ui'], 'show_in_rest' => true, 'show_tagcloud' => $item['show_tagcloud'], 'hierarchical' => $item['hierarchical']));
            });
        }
    }
}
