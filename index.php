...
if (get_option('headLine_random') != "checked") {
	...
	$args = array('posts_per_page' => 7, 'orderby' => 'rand', 'cat' => $minius_excludeIDs, 'ignore_sticky_posts' => true);
...
?><div class="col-xs-12"><div class="row"><?php
$listType = _opt('frontpage_postlist_type', 'lists');
?><div class="row postLists <?php
echo $listType;
?> <?php
echo _opt('enable_post_list_waterfall') ? 'waterfall' : '';
...
if ($like_num > 0) {
	$args = array(
	    ...
	    'meta_query' => array(array('key' => 'bigfa_ding', 'type' => 'NUMERIC')),
	    'orderby' => array('bigfa_ding' => 'DESC'),
	);
	$query_posts = new WP_Query();
	$query_posts->query($args);
...
foreach ($ids as $id) {
	$post = get_post($id);
	setup_postdata($post);
	?><div class="col-xxs-6 col-xs-4 col-lg-3 post-card-wrapper" style="overflow:hidden;"><?php
	include 'assets/template/postlist-post.php';
	?></div><?php
}
?></div></div><div class="row postLists"><?php
...
echo $rightClass;
?>"><div class="row"><div class="sidebar"><div manual-template="sidebarMenu"></div><div manual-template="sidebar" height-from="postLists"></div></div></div></div></div></div><div class="long-model-wrapper"><?php
...
foreach ($index_group as $group) {
	...
	$args = array('post_type' => 'post', 'cat' => $catIDs, 'orderby' => 'rand', 'posts_per_page' => $post_number, 'ignore_sticky_posts' => true);
	$query_posts = new WP_Query();
	$query_posts->query($args);
	$post_num_class = 'posts-' . $group['num'];
	?><div class="long-model <?php
	echo $post_num_class;
	?>"><div class="container"><span class="model-title"><?php
	echo $group['name'];
	?></span><?php
	if ($group['href']) {
		...
		echo $group['href'];
		?>"><?php
		echo $group['more'] ? $group['more'] : '阅读更多';
		?><i class="fas fa-angle-right"></i></a><?php
	}
	?><div class="row postLists <?php
	echo $group['type'];
	?>"><?php
	while ($query_posts->have_posts()) {
		$query_posts->the_post();
		?><div class="col-xxs-6 col-xs-4 col-md-3 post-card-wrapper"><?php
		include 'assets/template/postlist-post.php';
		?></div><?php
	}
	?></div></div></div><?php
